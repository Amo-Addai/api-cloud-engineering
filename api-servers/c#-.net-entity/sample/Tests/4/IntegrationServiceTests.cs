...
namespace Cadly.Slicer.API.Tests.Services
{
    ...
    public class IntegrationServiceTests
    {
		...
		[Fact]
		public void TestIntegrationServicesTests_Instantiated()
		{
        ...
        public async Task GetProjectAsync_ShouldReturnProjectOutputDto_WhenProjectExists()
        {
            // Arrange
            Guid projectId = Guid.NewGuid();
            string requestUrl = $"/projects/{projectId.ToString()}";
        ...
        public async Task StartManufacturerReviewAsync_ShouldReturnTrue_WhenUpdateIsSuccessful()
        {
            // Arrange
            Guid projectId = Guid.NewGuid();
            string requestUrl = $"/projects/{projectId.ToString()}/manufacturer-review";
        ...
        public async Task GetProjectFileAsync_ShouldReturnProjectFileOutputDto_WhenFileExists()
        {
            ...
            Guid projectId = Guid.NewGuid();
            Guid fileId = Guid.NewGuid();
            string requestUrl = $"/projects/{projectId.ToString()}/files/{fileId.ToString()}";
            HttpMethod requestMethod = HttpMethod.Get;
			...
			string expectedFilePath = "/" + $"{expectedFile.Name}.{expectedFile.Extension}";
            ...
            _httpMessageHandlerMock
				...
				.Setup<Task<HttpResponseMessage>>(
                    ...
                    ItExpr.Is<HttpRequestMessage>(
                        req =>
                            req.Method == requestMethod // also a GET Request
                            && !req.RequestUri.PathAndQuery.EndsWith(requestUrl)
                            && req.RequestUri.PathAndQuery.EndsWith(fileId.ToString())
                    ),
                    ItExpr.IsAny<CancellationToken>()
                )
                .Callback<HttpRequestMessage, CancellationToken>(
            ...
            Assert.IsType<ProjectFileOutputDto>(result.Item1);
            Assert.IsType<string>(result.Item2);
            Assert.Equal(expectedContent, JsonSerializer.Serialize<ProjectFileOutputDto>(result.Item1));
            Assert.Equal(expectedFilePath, result.Item2);
            Assert.Equal(expectedFile, result.Item1, Utils.JsonSerializerComparer<ProjectFileOutputDto>.Instance);
        ...
        public async Task SaveFileAsync_ShouldReturnTrue_WhenFileIsSavedSuccessfully()
        {
			...
			Guid fileId = Guid.NewGuid();
			HttpMethod requestMethod = HttpMethod.Get;
			string expectedFilePath = $"/{fileName}"; // Empty storage directory-path
            ...
            // Act
            var result = await _testService.SaveFileAsync(fileId, fileName, storagePath: ""); // todo: test with storagePath

            // Assert
            Assert.NotNull(result);
			Assert.IsType<string>(result);
			Assert.Equal(result, expectedFilePath);
        ...
        public async Task SaveFileAsync_ShouldReturnFalse_WhenFileIsNotSavedSuccessfully()
        {
			...
			Guid fileId = Guid.NewGuid();
			HttpMethod requestMethod = HttpMethod.Get;
			string expectedFilePath = ""; // Empty storage path, as a default value on failure
            ...
            // Act
            var result = await _testService.SaveFileAsync(fileId, fileName);
