...
export class EventsBridge implements OnApplicationBootstrap, OnApplicationShutdown {
    ...
    onApplicationBootstrap() {
         ...
         * we would periodically poll the event store for new events.
         * To keep track of what events we already processed, 
         * we would need to store the last processed event (cursor) in a separate collection
         */
        this.changeStream = this.eventStore
            .watch()
            .on(
                'change', (change: ChangeStreamInsertDocument<EventDocument>) => {
    ...
    handleEventStoreChange(change: ChangeStreamInsertDocument<EventDocument>) {
        /**
         * 'ChangeStreamInsertDocument' object exposes the 'txnNumber' property,
         * which represents the transaction identifier.
         * If you need multi-document transactions in your application,
