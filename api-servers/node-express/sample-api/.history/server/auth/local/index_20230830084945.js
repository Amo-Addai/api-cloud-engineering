...
async function contactRecipient(type, contactBody, defaultCmeth = false) {
    return new Promise(async (resolve, reject) => { // ERRORS CAN BE HANDLED FIRST OVER HERE
        try {
            const result = { code: 200, resultData: { success: true } }
            // await funct.contact(type, contactBody, defaultCmeth)
            resolve(result)
        } catch (err) {
            console.log("ERROR -> " + JSON.stringify(err))
            reject({ code: 400, resultData: { err, success: false, message: 'Sorry, some error occurred' } })
        }
    })
}
...
async function sendUserAuth(user) {
    return new Promise(async (resolve, reject) => { // ERRORS CAN BE HANDLED FIRST OVER HERE
        try {
            if (user && user._id) {
                const baseUrl = config.protocol + (config.ip === "localhost/" ? "localhost:" + config.port + "/" : config.ip),
                ...
                const result = await contactRecipient(recipientsType, contactBody, false)
...
function handleAuthenticationResults(res, err, user, extraAuth, info, userEdit = false) { // RETURN THE AUTH TOKEN
    ...
    if (!error && user) {
        console.log("\n\nuser: " + user)
        const token = auth.signToken(user, null)
        ...
        const resp = ({ err, err, success: false, message: info.message || "Something went wrong, please try again." })
...
router.post('/', auth.isAuthenticationAllowed(authOption, false), function (req, res, next) {
    ...
    const extraAuth = {} // req.extraAuthenticationData delete req.extraAuthenticationData
...
router.post('/signup', auth.isAuthenticationAllowed(authOption, true), auth.handleExtraAuthentication,
    async function (req, res, next) {
        try { // MAYBE, YOU SHOULD PERFORM SOME VALIDATIONS ON req.body.user OBJECT 1ST, BEFORE ADDING IT TO THE DATABASE
            ...
            const result = await modelscontrollersHandler.add(User, u) // DON'T RETURN USER DATA WITH ACCESS_TOKEN, LET USER LOGIN
    ...
    async function (req, res, next) {
        try {
            ...
            if (uid && uid.length > 0) { //  NOW, YOU CAN ACTIVATE THIS USER'S ACCOUNT
                const result = await modelscontrollersHandler.get(User, uid)
                console.log("RESULT -> " + JSON.stringify(result))
                if (result.code === 200) {
                    const user = result.resultData
...
router.post('/logout', async function (req, res, next) {
    ...
    try {
        if (req.body && (Object.keys(req.body).length > 0)) {
            const body = req.body
            const user = body.user, token = body.access_token, device = null // body.device
        ...
        console.log("ERROR -> " + JSON.stringify(err))
        return funct.sendResponse(res, 400, { err, success: false, message: "Some error occurred" })
    }
})
