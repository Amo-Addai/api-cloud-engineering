...
//
// const fileHandler = funct.fileHandler

const dataHandler = require('../utils/db/data-handler')
const modelscontrollersHandler = dataHandler.modelscontrollersHandler

const M = require('./user.model')

exports.M = M
exports.dataToExclude = M.dataToExclude
exports.imgdata = M.imgdata.length > 0
    ? imgdata
    : null
            ...
            .exec(async function (err, user) {
                try {
                    if (err) {
                        ...
                        const token = jwt.sign({ _id: user._id }, config.secrets.session, { expiresIn: config.tokenExpiration })
                        const extra = { "Email": null }
                        const contactData = {
                            user: [user],
                        ...
                        const result = await funct.contact('user', contactData, false)
                        return funct.sendResponse(res, result.code, result.resultData)
                    }
                } catch (err) {
                    console.log(err)
