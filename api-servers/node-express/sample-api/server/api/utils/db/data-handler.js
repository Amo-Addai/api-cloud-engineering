...
// const globalMM = require('../helpers/GlobalMarkModifiedServerFunctions')

// const filedata = 'data', storagebackup = 'storage'
...
function globalMarkModified (M, deepPop, data) {
    ...
    // const arr = globalMM.explode(deepPop), type = M.type
...
const DataHandler = {
    ...
    modelscontrollersHandler: {
        ...
        getAll: async function (type, condition) {
            return new Promise((resolve, reject) => {
                try {
                    // const M = DataHandler.models[type]
                    const M = type // NOW, type IS THE MODEL OBJECT ITSELF
                    ...
                    let extra = { limit: null, deepPop: true }
                ...
                if (body && Object.keys(body).length > 0) {
                    ...
                    M.Model.findById(id, M.dataToExclude, function (err, oldobj) {
                        if (err) reject({ code: 400, resultData: { err: err, success: false, message: 'Sorry, some error occurred' } })
                            ...
                            console.log('-----------------------------------------')
                            let updatedobj = markModified(oldobj, obj, M)
                            console.log('-----------------------------------------')
                ...
                if (true) {
                    ...
                    M.Model.findByIdAndRemove(id, function (err, oldobj) {
                        ...
                        if (err) reject({ code: 400, resultData: { err: err, success: false, message: 'Sorry, some error occurred' } })
                            ...
                            let deletedobj = JSON.parse(JSON.stringify(oldobj)) // DO IT THIS WAY TO AVOID SOME SILLY FUTURE ERROR!! TRUST ME :)
    ...
    updateDataSecurityEncryptionObject: async function (id) {
        return new Promise((resolve, reject) => {
            // const obj = {encryption: settings.getDataSecurityTechniqueSettings('encryption', 'data')}
            // const result = await this.modelscontrollersHandler.update('datasecurity', id, obj)
            resolve(true)
        })
    },
    ...
    getDataSecurityEncryptionObject: async function (id) {
        return new Promise((resolve, reject) => {
            // const result = await this.modelscontrollersHandler.get('datasecurity', id)
    ...
    encryptData: async function (type, schema) {
        return new Promise((resolve, reject) => {
            ...
            //     const mainData = this.modelcontrollers[type].mainData.split(' ')
            //     for (const prop of mainData) {
    ...
    decryptData: async function (type, schema) {
        return new Promise((resolve, reject) => {
            ...
            //         const encryption = await this.getDataSecurityEncryptionObject(schema['datasecurity']._id) || null
            //         if (encryption) {
            //             for (const prop of mainData) {
    ...
    deleteAllData: async function (store) {
        return new Promise((resolve, reject) => {
            // if (!store) store = settings.getStorageOrBackup(filedata, storagebackup)
            // for (const file in this.getAllData(store)) {
            //     switch (store) {
            //         case 'Internal':
            //             break
            //         case 'AWS Cloud':
