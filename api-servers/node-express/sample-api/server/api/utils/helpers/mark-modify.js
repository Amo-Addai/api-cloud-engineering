...
const mongooseHandler = require('../db/mongoose-handler')
...
module.exports = {
    ...
    handleDefault: async function (type, prop, data) {
        try {
            let id = '', M = null // THIS WILL BE THE MODEL OF THE OTHER OBJECT TO BE MARK MODIFIED
            if (this.isSingular(prop)) {
                ...
                const singularProp = getSingular(prop)
                ...
                if (data.deleted != undefined) {
                    ...
                    for (const a of [data["old"][prop], data[prop]] ) {// SO CALL THIS TO GET ALL POSSIBLE LINKS WITHIN THE ARRAY
                        for (const i of a) if (!arr.includes(i)) arr.push(i)
                    }
                    delete data["old"] // DO THIS IF YOU WANT, COZ IT WON'T BE NEEDED ANYMORE
                ...
                console.log("NOW ARRAY TO GLOBALLY MARK MODIFY -> ")
                console.log(arr)
    ...
    handleSingleOrPlural: async function (OtherM, id, type, data, otherType) {
        return new Promise((resolve, reject) => {
            ...
            let aimToSet = false
            const pluralOtherType = getPlural(otherType), ModifyingM = this.getModel(type)
            //
            ...
            OtherM.findById(id, function (err, old) { // THIS FUNCTION WILL NOT DEEP POPULATE
                if (err || !old) {
                    ...
                    const pluralType = getPlural(type), modifyingId = data._id
                    ///////////////////////////////////////////////////////////////////////////////////////////////
                    ...
                    const updatedobj = old // funct.markModified(old, {}) // NOT EVEN REQUIRED SINCE THERE'S NO NEW OBJECT
